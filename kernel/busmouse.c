...
static int busmouse_release(struct inode *inode, struct file *file)
{
	...
	// 3. 因此，在 SMP 上可能會是 reentrant function
	//    但 spin_lock 是 busy-waiting 實作，process 無法 go to sleep
	lock_kernel();
	busmouse_fasync(-1, file, 0);

	// 1. 再減 1 為 0 時，就 callback 下層的 close driver function
	if (--mse->active == 0) {
		if (mse->ops->release)
			// 2. 無法預期下層的行為
			//    例如：可能使用到 Blocking APIs
